...
public class CharacterEncodingFilter implements Filter {
    ...
    *  4. 파라미터로 주어지는 FilterConfig 객체에서 필터 값 갖고옴 
    */
   // 필터 초기화 메소드
     @Override
   ...
   /*
    * [필터기능 수행]
    *  1. 필터에서 지정한 url-pattern에 정한("/*") 요청이 올때마다 
         실행됨.
   ...
   public void doFilter(ServletRequest request, 
      ...
      System.out.println("여기는 서블릿 필터의 doFilter "
            + "메소드로 서블릿들이 호출될때마다 호출됨.");

      /*
       * [서블릿이 실행되기 전에 해야할 작업]
       *  1. config.getInitParameter("encoding") 값은
       *   - web.xml에서 <param-name>으로 설정한 값 즉, 
            utf-8
       */
